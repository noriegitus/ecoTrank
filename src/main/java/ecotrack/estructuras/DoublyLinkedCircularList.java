...
public class DoublyLinkedCircularList<E> implements List<E>, Serializable {
    ...
    private class IteradorCircular implements Iterator<E> {
        ...
        public E previous() {
            if (isEmpty()) throw new NoSuchElementException("La lista está vacía");
            if (contador <= 0) {
                // Ya estamos al principio o antes del inicio, no podemos ir más atrás
                throw new NoSuchElementException("No hay elementos anteriores disponibles");
            }
            actual = actual.prev; 
            contador--; 
    ...
    // --- Métodos especificos del proyecto (Residuos) ---
    ...
    public E buscarResiduo(String id) {
        if (isEmpty() || id == null || id.trim().isEmpty()) return null;
        Node actual = head;
        
        // Recorremos hasta dar la vuelta
        do {
            // Verificamos instanceof para seguridad de tipos y validamos nulls
            if (actual.content != null && actual.content instanceof Residuo) {
                Residuo r = (Residuo) actual.content;
                String residuoId = r.getId();
                if (residuoId != null && residuoId.equals(id)) {
                    return actual.content;
                }
            }
            actual = actual.next;
    ...
    public boolean eliminarResiduo(E r) {
        ...
        Node actual = head;
        do {
            // Validar que el contenido no sea null antes de llamar a equals
            if (actual.content != null && actual.content.equals(r)) {
                if (size == 1) {
                    head = tail = null;
                    size = 0;
                    return true;
    ...
    public void sort(Comparator<E> c) {
        ...
        do {
            ...
            for (int i = 0; i < size - 1; i++) {
                ...
                // Validar que ninguno de los contenidos sea null antes de comparar
                if (actual.content == null || siguiente.content == null) {
                    actual = actual.next;
                    continue;
                }
                try {
                    ...
                    // Si hay error en la comparación, continuar con el siguiente par
                    // Esto previene que el sort falle completamente
                }
                actual = actual.next;
            }
        } while (cambiado);

...
public class ZonasController implements SistemaInyectable {
    ...
    private void recolectarZona() {
        try {
            if (sistema == null) {
                ...
                alert.setContentText("Error: El sistema no está inicializado.");
                alert.show();
                return;
            }
            ...
            if (cola == null) {
                ...
                alert.setTitle("Error");
                alert.setHeaderText(null);
                alert.setContentText("Error: La cola de zonas no está inicializada.");
                alert.show();
                return;
            }
            ...
            if (z == null) {
                ...
                alert.setHeaderText(null);
                alert.setContentText("No hay zonas registradas");
                alert.show();
                return;
            }
            ...
            alert.setHeaderText(null);
            alert.setContentText("Zona '" + z.getNombreZona() + "' recolectada");
            alert.show();
            ...
            alert.setTitle("Error");
            alert.setHeaderText(null);
            alert.setContentText("Error al recolectar la zona: " + e.getMessage());
            alert.show();
        }
    }
    ...
    private void recolectarZonaSeleccionada() {
        try {
            ...
            if (seleccionada == null) {
                ...
                alert.setTitle("Advertencia");
                alert.setHeaderText(null);
                alert.setContentText("Seleccione una zona de la tabla para recolectar.");
                alert.show();
                return;
            }
            ...
            alert.setTitle("Información");
            alert.setHeaderText(null);
            alert.setContentText("Zona '" + seleccionada.getNombreZona() + "' recolectada correctamente.");
            alert.show();
    ...
    private void eliminarZona() {
        ...
        if (seleccionada == null) {
            ...
            alert.setTitle("Advertencia");
            alert.setHeaderText(null);
            alert.setContentText("Seleccione una zona de la tabla para eliminar.");
            alert.show();
            return;
        }
        ...
        if (seleccionada.getP_Pendiente() > 0) {
            ...
            alert.setTitle("Error");
            alert.setHeaderText(null);
            alert.setContentText("No se puede eliminar la zona '" + seleccionada.getNombreZona() + 
                "' porque tiene " + seleccionada.getP_Pendiente() + 
                " kg de residuos pendientes.");
            alert.show();
            return;
        }
        ...
        confirmacion.showAndWait().ifPresent(result -> {
            if (result == ButtonType.OK) {
                ...
                if (eliminada) {
                    ...
                    alert.setTitle("Información");
                    alert.setHeaderText(null);
                    alert.setContentText("Zona '" + seleccionada.getNombreZona() + "' eliminada correctamente.");
                    alert.show();
                } else {
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Error");
                    alert.setHeaderText(null);
                    alert.setContentText("No se pudo eliminar la zona.");
                    alert.show();
                }
            }
        });
    ...
    private void agregarZona() {
        ...
        try {
                ...
                try {
                    if (nombreZona != null && !nombreZona.trim().isEmpty()) {
                        ...
                        if (sistema.getMapaZonas().containsKey(nombreTrim)) {
                            ...
                            alert.setTitle("Advertencia");
                            alert.setHeaderText(null);
                            alert.setContentText("La zona '" + nombreTrim + "' ya existe.");
                            alert.show();
                            return;
                        }
                        ...
                        alert.setTitle("Información");
                        alert.setHeaderText(null);
                        alert.setContentText("Zona '" + nombreTrim + "' agregada correctamente.");
                        alert.show();
                    } else {
                        Alert alert = new Alert(Alert.AlertType.WARNING);
                        alert.setTitle("Advertencia");
                        alert.setHeaderText(null);
                        alert.setContentText("El nombre de la zona no puede estar vacío.");
                        alert.show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    ...
                    alert.setTitle("Error");
                    alert.setHeaderText(null);
                    alert.setContentText("Error al agregar la zona: " + e.getMessage());
                    alert.show();
                }
            });
        } catch (Exception e) {
            ...
            alert.setTitle("Error");
            alert.setHeaderText(null);
            alert.setContentText("Error al abrir el diálogo: " + e.getMessage());
            alert.show();
        }
    }
}

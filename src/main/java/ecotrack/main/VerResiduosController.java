...
public class VerResiduosController implements SistemaInyectable {
    ...
    private void eliminarResiduo() {
        if (tablaResiduos == null || sistema == null) {
            mostrarAlerta(Alert.AlertType.ERROR, "Error", "El sistema no está inicializado correctamente.");
            return;
        }
        ...
        if (seleccionado == null) {
            mostrarAlerta(Alert.AlertType.WARNING, "Advertencia", "Seleccione un residuo para eliminar.");
            return;
        }

        if (sistema.getListaResiduos() == null) {
            mostrarAlerta(Alert.AlertType.ERROR, "Error", "La lista de residuos no está disponible.");
            return;
        }
        ...
        if (eliminado) {
            ...
            this.iteradorActual = sistema.getListaResiduos().iterator();
            mostrarAlerta(Alert.AlertType.INFORMATION, "Éxito", "Residuo eliminado correctamente.");
        } else {
            mostrarAlerta(Alert.AlertType.ERROR, "Error", "No se pudo eliminar el residuo.");
        }
    }
    ...
    private void agregarResiduo() {
            ...
            if (dialogButton == ButtonType.OK) {
                try {
                    ...
                    try {
                        ...
                        mostrarAlerta(Alert.AlertType.ERROR, "Error", "El peso debe ser un número válido.");
                        return null;
                    }
                    
                    // Validar que el peso sea positivo
                    if (peso <= 0) {
                        mostrarAlerta(Alert.AlertType.ERROR, "Error", "El peso debe ser mayor que cero.");
                        return null;
                    }

                    // Validar y parsear prioridad
                    int prioridad;
                    try {
                        prioridad = Integer.parseInt(prioridadStr);
                    } catch (NumberFormatException e) {
                        mostrarAlerta(Alert.AlertType.ERROR, "Error", "La prioridad debe ser un número entero válido.");
                        return null;
                    }
                    
                    // Validar rango razonable para prioridad (1-10)
                    if (prioridad < 1 || prioridad > 10) {
                        mostrarAlerta(Alert.AlertType.ERROR, "Error", "La prioridad debe estar entre 1 y 10.");
                        return null;
                    }
                    ...
                    try {
                        ...
                        mostrarAlerta(Alert.AlertType.ERROR, "Error", "Error al crear el residuo: " + e.getMessage());
                        return null;
                    }
                } catch (Exception e) {
        ...
        dialog.showAndWait().ifPresent(result -> {
            if (result != null && result.getValue() != null) {
                ...
                mostrarAlerta(Alert.AlertType.INFORMATION, "Éxito", "Residuo agregado correctamente.");
            }
        });
    }
